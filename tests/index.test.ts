...
describe('MCP Agent Tools', () => {
  ...
  describe('Pipeline Tool', () => {
    ...
    it('should filter by branch', async () => {
      const result = await handleGetPipelineStatus({ branch: 'main' });
      expect(result).toContain('main');
    });

    it('should filter by status', async () => {
      const result = await handleGetPipelineStatus({ status: 'failure' });
      expect(result).toContain('FAILURE');
    });

    it('should return no runs message if none match', async () => {
      const result = await handleGetPipelineStatus({ branch: 'non-existent-branch' });
      expect(result).toEqual('No pipeline runs found matching the given filters.');
    });
  });

  describe('Vulnerability Tool', () => {
    it('should return all vulnerabilities when no filters applied', async () => {
      const result = await handleTriageVulnerabilities({});
      expect(result).toContain(`Found ${mockVulnerabilities.length} vulnerability(ies)`);
    });

    it('should filter by severity', async () => {
      const result = await handleTriageVulnerabilities({ severity: 'critical' });
      expect(result).toContain('CRITICAL');
    });

    it('should return no vulnerabilities message if none match', async () => {
      const result = await handleTriageVulnerabilities({ severity: 'unknown' });
  ...
  describe('Logs Tool', () => {
    ...
    it('should filter by service', async () => {
      const result = await handleSearchLogs({ service: 'auth-service' });
      expect(result).toContain('auth-service');
    });

    it('should filter by keyword', async () => {
      const result = await handleSearchLogs({ keyword: 'token' });
      expect(result).toContain('token');
    });

    it('should return no logs message if none match', async () => {
      const result = await handleSearchLogs({ service: 'non-existent-service' });
      expect(result).toEqual('No log entries found matching the given filters.');
    });
  });

  describe('Dependency Tool', () => {
    it('should return all dependencies if no min severity', async () => {
      const result = await handleScanDependencies({});
      expect(result).toContain(`Dependency scan found ${mockDependencyVulns.length} vulnerable package(s)`);
    });

    it('should filter by min_severity', async () => {
      const result = await handleScanDependencies({ min_severity: 'high' });
      expect(result).toContain('HIGH');
    });

    it('should filter by min_severity (critical)', async () => {
      const result = await handleScanDependencies({ min_severity: 'critical' });
      expect(result).toContain('CRITICAL');
    });

    it('should return no issues message if none match', async () => {
  ...
  describe('Kubernetes Tools', () => {
    it('should register k8s tools', async () => {
